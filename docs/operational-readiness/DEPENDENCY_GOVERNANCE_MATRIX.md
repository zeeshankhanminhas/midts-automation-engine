...
## Build Start Criteria (When to Start Building)

Start new workflow implementation only when all conditions below are met:

1. **All Blocked dependencies are resolved to Verified** in this matrix.
2. **No critical Settings keys are missing** in the active runtime environment.
3. **Governance reference exists**: `docs/core-master-framework/00_RESTRUCTURE_MAP.md` is present in git.
4. **Vendor pricing governance is documented and approved** (schema + margin contract).
5. **Verification evidence is current**: evidence records captured within the last 7 calendar days.
6. **Gate approval is recorded** by MIDTS owner/approver in release notes.

### Minimum Go/No-Go Rule
- **Go**: Every dependency row is `Verified`, or explicitly accepted as temporary risk by MIDTS approver with expiration date.
- **No-Go**: Any dependency remains `Blocked` without an approved written exception.

### Suggested First Build Scope After Gate Approval
After the gate is approved, begin with the smallest controlled implementation slice:
1. Vendor Pricing sheet header alignment only.
2. Margin calculation contract enforcement tests.
3. Quote generation dependency enforcement checks.

Do not bundle unrelated feature work into the first post-gate build.
