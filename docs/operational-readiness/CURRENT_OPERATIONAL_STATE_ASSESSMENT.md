...
| Quote generation | Gated and price-governed in current code; template approval/wiring incomplete | Approved pricing path uses final customer price from vendor pricing; in-repo quote templates exist, but live document output and trigger wiring remain unfinished. |
...
| Email | Implemented but config/template-dependent | Brevo API/sender keys and form URLs are required; in-repo production templates exist, but live Brevo/template approval and send wiring still need proof. |
...
- Business approval of the in-repo client quote template and any external Doc/PDF equivalent.
- Quote output folder policy.
- Customer-facing quote content review.
- Hooking quote document generation into the approved quote lifecycle if formal documents are required.
...
### 4.8 Template readiness is partially implemented but not production-approved

`ProductionTemplateService` now provides an in-repo catalogue for lifecycle email and document templates, and `ProductionLifecycleSafetyCheck` can audit those templates without sending email or writing Drive files. Remaining production-readiness work is approval and wiring, not a blank-template gap:

- Confirm business approval for template wording, merge fields, recipients, and owner.
- Decide whether in-repo rendered documents are sufficient or whether external Google Doc/PDF template IDs are required.
- Wire document generation triggers into the quote/project/payment lifecycle where approved.
- Confirm output folder policy, naming convention, and Drive logging for generated documents.
- Add or approve NDA/contract/e-sign templates if those are required before vendor access or customer acceptance.
...
- Production lifecycle templates and dry-run/safety-check runners exist for quote accepted, payment request, project started, file received, delivery ready, revision clarification, and unable-to-quote messages.
...
## 7. Template and Document Readiness

The repository now contains in-repo production template catalogues and lifecycle dry-run helpers, so the remaining issue is **governed production approval and runtime wiring**, not the total absence of templates. Current readiness status:

| Template/document area | Current repo state | Remaining action |
| --- | --- | --- |
| Lifecycle emails | In-repo templates exist in `ProductionTemplateService` | Approve copy, recipients, and Brevo/live send wiring. |
| Client quote document | In-repo `CLIENT_QUOTE` document template exists | Decide whether rendered in-repo template is enough or an external Google Doc/PDF template is required; wire generation trigger. |
| Scope of Work | In-repo `SCOPE_OF_WORK` document template exists | Approve business content and output folder policy. |
| Project handoff brief | In-repo `PROJECT_HANDOFF_BRIEF` document template exists | Wire only after project/Drive policy is confirmed. |
| Payment record | In-repo `PAYMENT_RECORD` document template exists | Decide whether this satisfies receipt/invoice needs or a payment-provider/invoice template is required. |
| Delivery/project closure documents | In-repo lifecycle document templates exist | Approve trigger points and Drive logging policy. |
| Vendor NDA / customer contract / e-sign pack | Not fully governed in repo | Add/approve only if required before vendor access or customer acceptance. |
| Drive folder taxonomy / folder README | Not fully governed in repo | Define taxonomy, naming, inheritance, and README template before broad Drive automation. |
| Internal admin readiness checklist | This assessment provides the checklist baseline | Convert into an operator checklist or release-runbook if needed. |
...
- `QUOTE_TEMPLATE_ID` or equivalent quote document template key, only if an external Google Doc/PDF template is required instead of the in-repo `CLIENT_QUOTE` template.
- `QUOTE_OUTPUT_FOLDER_ID` or documented use of an existing controlled folder.
- `PAYMENT_RECEIPT_TEMPLATE_ID` or invoice/receipt template key, only if `PAYMENT_RECORD` is not sufficient.
- `PROJECT_KICKOFF_TEMPLATE_ID`, only if external project-kickoff documents are required beyond lifecycle email/document templates.
...
5. Production communication/template approval and live wiring are incomplete, even though in-repo templates exist.
6. Quote document/PDF/output-folder governance and trigger wiring are incomplete.
...
6. Approve quote/payment/project/vendor templates and decide whether external document IDs are required before customer-facing document automation.
...
- [ ] Run `runProductionLifecycleSafetyCheck()` to verify lifecycle template wiring without sending emails or writing Drive files.
- [ ] Run `auditProductionCommunicationTemplates()` to check in-repo templates for unsafe wording before live approval.
...
- [ ] `runProductionLifecycleSafetyCheck()`
- [ ] `auditProductionCommunicationTemplates()`
...
- [ ] `runProductionLifecycleDryRuns()` with representative values before any live lifecycle email send
- [ ] `runProductionLifecycleSmokeTest()` only after `runProductionLifecycleSafetyCheck()` passes
...
- [ ] Approve quote, payment, and project communication templates, including whether in-repo templates are sufficient or external document templates are needed.
...
| Quote/document mismatch | High | Medium | Quote amount path is improved and in-repo templates exist, but live document generation/output governance is incomplete. | Approve in-repo templates or external Doc/PDF templates, then define output-folder governance before production quote delivery. |
...
However, the system is **not yet production-proven**. The immediate next step should be readiness verification, not new feature work. In particular, verify live Settings, Apps Script deployment, website routing, Sheet headers, Drive permissions, Brevo/Slack configuration, dashboard browser behavior, in-repo template safety checks, and any required external document-template readiness. Then decide the payment-before-project policy and central audit approach before making the next runtime change.
