 ...
 * SIDE EFFECTS: Appends synthetic test rows to Leads, Vendors, Vendor Pricing, Quotes, Projects, Payments, Drive logs, and Error Logs.
 */
function runCommercialWorkflowSmokeTest(options) {
  ...
  try {
    ...
    // Write a Drive audit marker immediately after setup so Drive Logs prove the smoke test ran,
    // even if a later lead/vendor/quote/payment gate fails before real Drive work is reached.
    var driveAuditStart = DriveService.logDriveAccess_(
      'COMMERCIAL_SMOKE_TEST_STARTED',
      '',
      '',
      '',
      '',
      'Started',
      testTag + ' started in ' + (testMode ? 'TEST_MODE; no live Drive folder will be created unless createDriveFolder=true.' : 'live mode.')
    );
    if (!driveAuditStart.success) {
      return driveAuditStart;
    }
    ...
    return {
      ...
      data: {
        ...
        workflowSummary: {
          ...
          driveAuditStartStatus: driveAuditStart.message,
        ...
        steps: {
          setup: setup,
          driveAuditStart: driveAuditStart,
          lead: lead,
